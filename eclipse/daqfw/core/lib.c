...
// 7.5.5   Code
   ...
   i = MAP_Timer32_getValue(TIMER32_0_BASE);
   // Spin
   while (i - MAP_Timer32_getValue(TIMER32_0_BASE) < j);
...
// 7.6.5   Code
   ...
   if (op & CLK_START) {
      *tNow = MAP_Timer32_getValue(TIMER32_0_BASE);
   }
   // Report the Count,
   // Read the 32-Bit Time Base Register
   else if (op & CLK_STOP) {
      tEnd = MAP_Timer32_getValue(TIMER32_0_BASE);
      switch (units) {
         case MICROSECONDS:
