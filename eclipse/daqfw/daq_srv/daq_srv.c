...
// 7.4.5   Code
   ...
   //
      ...
      // packet ready indication, create pipe message
      if (msg->p.flags & DAQ_INT_FLAG_PKT) {
         memset(&pipe, 0, sizeof(cm_pipe_t));
         pipe.dst_cmid = CM_ID_PIPE;
         pipe.msgid    = CM_PIPE_DAQ_DATA;
         pipe.port     = 0;
         pipe.flags    = 0;
         pipe.seqid    = sv.seqid++;
         pipe.stamp    = gc.sys_time;
         pipe.stamp_us = 0;
         pipe.magic    = 0x55AA1234;
         // transfer samples to message body
         uint16_t *adc_sam = adc_sam_ptr();
         for (i=0;i<DAQ_MAX_LEN;i++) {
            pipe.samples[i] = adc_sam[i];
         }
         // send the pipe message direct, bypass CM
         uart_pipe(&pipe, sizeof(cm_pipe_t));
      }
